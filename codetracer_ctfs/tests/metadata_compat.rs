...
fn test_metadata_compat_read_nim_ct() {
    ...
    let mut reader =
        codetracer_ctfs::CtfsReader::open(std::path::Path::new(&ct_path)).unwrap_or_else(|e| panic!("failed to open Nim .ct file: {}", e));
    ...
    {
        let data = reader
            .read_file("filemap.bin")
            .unwrap_or_else(|e| panic!("failed to read filemap.bin: {}", e));
        eprintln!("filemap.bin: {} bytes", data.len());
        let fm = Filemap::deserialize(&data).unwrap_or_else(|e| panic!("failed to deserialize filemap.bin: {}", e));
        verify_filemap(&fm);
    ...
    {
        let data = reader
            .read_file("platform.bin")
            .unwrap_or_else(|e| panic!("failed to read platform.bin: {}", e));
        eprintln!("platform.bin: {} bytes", data.len());
        let p = PlatformInfo::deserialize(&data).unwrap_or_else(|e| panic!("failed to deserialize platform.bin: {}", e));
        verify_platform(&p);
    ...
    {
        let data = reader.read_file("mmap.bin").unwrap_or_else(|e| panic!("failed to read mmap.bin: {}", e));
        eprintln!("mmap.bin: {} bytes", data.len());
        let t = MmapTable::deserialize(&data).unwrap_or_else(|e| panic!("failed to deserialize mmap.bin: {}", e));
        verify_mmap(&t);
...
fn test_metadata_compat_read_nim_raw_files() {
    ...
    if let Ok(path) = std::env::var("METADATA_COMPAT_NIM_PLATFORM") {
        let nim_data = std::fs::read(&path).unwrap_or_else(|e| panic!("failed to read {}: {}", path, e));
        let p = PlatformInfo::deserialize(&nim_data).unwrap_or_else(|e| panic!("failed to deserialize Nim platform: {}", e));
        verify_platform(&p);
    ...
    if let Ok(path) = std::env::var("METADATA_COMPAT_NIM_MMAP") {
        let nim_data = std::fs::read(&path).unwrap_or_else(|e| panic!("failed to read {}: {}", path, e));
        let t = MmapTable::deserialize(&nim_data).unwrap_or_else(|e| panic!("failed to deserialize Nim mmap: {}", e));
        verify_mmap(&t);
