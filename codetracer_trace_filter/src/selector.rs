...
//!
//! Mirrors the selector grammar defined in
...
//! ```
//!
//! When `<match_type>` is omitted the matcher defaults to `glob`.
...
/// Selector kinds defined by the cross-language spec § 4.
///
/// Tier-1 kinds (`Package`, `File`, `Object`) are valid in `[[scope.rules]]`;
/// Tier-2 kinds (`Local`, `Global`, `Arg`, `Return`, `Attr`) are valid in
...
impl SelectorKind {
    /// Return the token used in selector strings (matches spec § 4 verbatim).
...
/// Match strategy configured for a selector.  Defaults to `Glob` when
/// `<match_type>` is omitted from the selector string (spec § 4).
...
/// Parsed selector with a compiled matcher.
#[derive(Debug, Clone)]
pub struct Selector {
    raw: String,
    kind: SelectorKind,
...
impl Selector {
    /// Parse a selector string, optionally restricting allowed kinds.
    ///
    /// When `permitted_kinds` is empty, all selector kinds are accepted.
    pub fn parse(raw: &str, permitted_kinds: &[SelectorKind]) -> FilterResult<Self> {
        if raw.is_empty() {
            return Err(filter_invalid!("selector string is empty"));
        }
        ...
        if pattern.is_empty() {
            return Err(filter_invalid!("selector pattern cannot be empty"));
        }
...
fn build_matcher(match_type: MatchType, pattern: &str) -> FilterResult<Matcher> {
    match match_type {
        ...
        MatchType::Glob => {
            let glob = GlobBuilder::new(pattern)
                ...
                .build()
                .map_err(|err| filter_invalid!("invalid glob pattern '{}': {}", pattern, err))?;
...
fn log_regex_failure(pattern: &str, err: &RegexError) {
    ...
    log::warn!(
        target: "codetracer_trace_filter::selector",
